...
- **Free Spins**: Triggered by eight 🏰 symbols in the middle row (or any row if Bonus Spin Chance is active) or by purchasing from the store, granting 5 free spins (`FREE_SPINS = 5`). Free spins use the current bet for payouts but do not deduct from the balance.
...
**Example**: If you hit two paylines in a single spin (e.g., one 🗡️ and one 🔥), you might earn `floor(10 × 1) = 10 coins` (🗡️) + `floor(10 × 3) = 30 coins` (🔥) = 40 coins, plus, for example, 50 credits for the 🗡️ payline and 30 credits for the 🔥 payline, totaling 80 credits.

### Free Spins Mode
- **Trigger**: Activated by eight 🏰 symbols in the middle row (row 5, index 4, checked via `check_bonus`) or in any row if Bonus Spin Chance is active (purchased for 100 credits, applies to the next spin only). Can also be triggered by purchasing "Free Spins Purchase" from the store.
...
- **Bonus Spin Chance**: When active, increases the chance of triggering free spins by checking all rows for eight 🏰 symbols (effectively a ~10% increased probability, as it checks 8 rows instead of 1). Resets after the next spin.
...
- **Bonus Spin Chance** (100 credits): Increases the chance of triggering free spins by checking all rows for eight 🏰 symbols on the next spin (effectively ~10% higher probability).
...
- **Leaderboard**: Stores the top 5 high scores (balance) with player names in `leaderboard.json`. Updated on game quit, when the balance reaches zero ( estudiante over), or after a jackpot win.
...
- **Spin**:
  ...
  - Free spins trigger on eight 🏰 symbols (middle row or any row with Bonus Spin Chance) or store purchase, running 5 spins.
...
- **Bonus Spin Chance Purchase**:
  ...
  - Activates Bonus Spin Chance for the next spin, checking all rows for eight 🏰 symbols.
...
- **Free Spins** (triggered by eight 🏰 in any row due to Bonus Spin Chance):
  - 5 spins, each checking paylines with 10-coin bet.
  - Spin 1: Hits 🗡️ payline (`floor(10 × 1) = 10 coins`, 50 credits). Total so far: 10 coins, 50 credits.
...
This README provides a complete overview of the 8x8 slot machine game, covering payouts, extra credits, the Mystery Prize, Bonus Spin Chance, and all mechanics for regular and free spins, ensuring players understand how to play and what to expect from wins.