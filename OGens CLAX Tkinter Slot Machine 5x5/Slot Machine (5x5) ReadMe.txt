...
- **Paylines**: 25 predefined paylines (horizontal, vertical, diagonal, and patterns like V-shape, zigzag, and W-shape) checked for wins.
- **Currency**: 
...
- **Free Spins**: Triggered by five 💎 symbols in the middle row (or any row if Bonus Spin Chance is active) or by purchasing from the store, granting 5 free spins (`FREE_SPINS = 5`). Free spins use the current bet for payouts but do not deduct from the balance.
...
The multipliers are defined in the `PAYOUTS` dictionary (halved from original values):  
- 🍒: 1x  
- 🔔: 2x  
- 🍋: 1x  
- 7️⃣: 4x  
...
For paylines with fewer than 5 symbols, the multiplier is halved (integer division).  
**Example**: If your bet is 10 coins and you hit a 5-symbol payline with 🍒 symbols, you win `floor(10 × 1) = 10 coins`. For a 3-symbol payline, you win `floor(10 × (1 // 2)) = 0 coins`.  
...
### Free Spins Mode
- **Trigger**: Activated by five 💎 symbols in the middle row (row 3, index 2, checked via `check_bonus`) or in any row if Bonus Spin Chance is active (purchased for 100 credits, applies to the next spin only). Can also be triggered by purchasing "Free Spins Purchase" from the store.
...
- **Bonus Spin Chance**: When active, increases the chance of triggering free spins by checking all rows for five 💎 symbols (effectively a ~10% increased probability, as it checks 5 rows instead of 1). Resets after the next spin.
...
- **Bonus Spin Chance** (100 credits): Increases the chance of triggering free spins by checking all rows for five 💎 symbols on the next spin (effectively ~10% higher probability).
...
- **Spin**:
  ...
  - Free spins trigger on five 💎 symbols (middle row or any row with Bonus Spin Chance) or store purchase, running 5 spins.
...
- **Bonus Spin Chance Purchase**:
  ...
  - Activates Bonus Spin Chance for the next spin, checking all rows for five 💎 symbols.
...
- **Free Spins** (triggered by five 💎 in any row due to Bonus Spin Chance):
...
This README provides a complete overview of the 5x5 slot machine game, covering payouts, extra credits, the Mystery Prize, Bonus Spin Chance, and all mechanics for regular and free spins, ensuring players understand how to play and what to expect from wins.