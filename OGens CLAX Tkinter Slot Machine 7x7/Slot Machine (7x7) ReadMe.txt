# Slot Machine Game README (7x7 Version)

## Overview
This is an advanced Python-based slot machine game with a 7x7 grid, featuring 40 paylines, a variety of symbols, and engaging mechanics such as regular spins, free spins, a jackpot, extra credits, and an in-game store. Built using `tkinter` for the GUI and `secrets` for true randomness, the game offers an immersive experience with persistent player data, a leaderboard, and customizable bets. Players can win coins and credits, track stats, and purchase items, including a Mystery Prize and Bonus Spin Chance, to enhance gameplay.

## Game Features

### Game Setup
- **Grid**: 7 reels × 7 rows, displaying 49 symbols per spin.
- **Symbols**: 🍒 (Cherry), 🔔 (Bell), 🍋 (Lemon), 7️⃣ (Seven), ⭐ (Star), 💎 (Diamond, also the wild symbol), 🍉 (Watermelon), 🍊 (Orange).
- **Paylines**: 30 predefined paylines (horizontal, vertical, diagonal, and patterns like V-shape, zigzag, W-shape, and inverted V-shape) checked for wins.
- **Currency**: 
...
- **Free Spins**: Triggered by seven 💎 symbols in the middle row (or any row if Bonus Spin Chance is active) or by purchasing from the store, granting 5 free spins (`FREE_SPINS = 5`). Free spins use the current bet for payouts but do not deduct from the balance.
...
For paylines with fewer than 7 symbols, the multiplier is halved (integer division).  
**Example**: If your bet is 10 coins and you hit a 7-symbol payline with 🍒 symbols, you win `floor(10 × 1) = 10 coins`. For a 4-symbol payline, you win `floor(10 × (1 // 2)) = 0 coins`.  
...
### Extra Credits
For each winning payline, you also receive **random extra credits** (between 1 and 50, generated using `secrets.randbelow(50) + 1`).  
These credits are added to your total credits, which can be used to purchase items in the store (e.g., extra spins, balance boosts, Mystery Prize, or Bonus Spin Chance).

**Example**: If you hit two paylines in a single spin (e.g., one 🍒 and one ⭐), you might earn `floor(10 × 1) = 10 coins` (🍒) + `floor(10 × 3) = 30 coins` (⭐) = 40 coins, plus, for example, 50 credits for the 🍒 payline and 30 credits for the ⭐ payline, totaling 80 credits.

### Free Spins Mode
- **Trigger**: Activated by seven 💎 symbols in the middle row (row 4, index 3, checked via `check_bonus`) or in any row if Bonus Spin Chance is active (purchased for 100 credits, applies to the next spin only). Can also be triggered by purchasing "Free Spins Purchase" from the store.
...
- **Bonus Spin Chance**: When active, increases the chance of triggering free spins by checking all rows for seven 💎 symbols (effectively a ~10% increased probability, as it checks 7 rows instead of 1). Resets after the next spin.
- **Winnings**: Payouts from paylines and extra credits (1–50 per winning payline) accumulate over the 5 spins. Total coins are added to the balance, and credits are added to the credit balance at the end.
...
- **Bonus Spin Chance** (100 credits): Increases the chance of triggering free spins by checking all rows for seven 💎 symbols on the next spin (effectively ~10% higher probability).
...
### Game Interface
- **GUI**: Built with `tkinter`, featuring a 7x7 grid of reel labels, balance/credits/jackpot/bet/extra spins displays, a bet adjustment entry/buttons, a store listbox, payline/status displays, and buttons for Spin, Quit, Leaderboard, and Reset. The window size is 800x600 pixels.
- **Visual Feedback**:
  ...
  - Status messages indicate wins (green), losses (red), free spins (blue), or jackpots (purple). Bonus Spin Chance activation is shown in blue.
...
- **Spin**:
  ...
  - Free spins trigger on seven 💎 symbols (middle row or any row with Bonus Spin Chance) or store purchase, running 5 spins.
...
- **Randomness**: Uses `secrets` for secure random symbol selection, credit generation, jackpot checks, Mystery Prize outcomes, and Bonus Spin Chance mechanics.
...
- **Regular Spin**:
  ...
  - Hits two paylines (🍒: `floor(10 × 1) = 10 coins`, ⭐: `floor(10 × 3) = 30 coins`) and earns 50 + 30 = 80 credits.
...
- **Bonus Spin Chance Purchase**:
  - Spend 100 credits (credits: 0 or 65, depending on Mystery Prize).
  - Activates Bonus Spin Chance for the next spin, checking all rows for seven 💎 symbols.
- **Extra Spin** (if awarded by Mystery Prize):
  - Uses 1 extra spin (extra spins: 0), no coins deducted.
  - Hits ⭐ payline (`floor(10 × 3) = 30 coins`, 40 credits). Balance: 240, credits: 40 or 105.
- **Free Spins** (triggered by seven 💎 in any row due to Bonus Spin Chance):
  - 5 spins, each checking paylines with 10-coin bet.
  - Spin 1: Hits 🍒 payline (`floor(10 × 1) = 10 coins`, 50 credits). Total so far: 10 coins, 50 credits.
  - Spin 2–5: No wins. Final: 10 coins added to balance (now 250), 50 credits added (now 90 or 155).
- **Store Purchase**: Spend 100 credits on Balance Boost, adding 100 coins (balance: 350, credits: 0 or 55).

This README provides a complete overview of the 7x7 slot machine game, covering payouts, extra credits, the Mystery Prize, Bonus Spin Chance, and all mechanics for regular and free spins, ensuring players understand how to play and what to expect from wins.